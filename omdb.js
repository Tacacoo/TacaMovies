$(document).ready(function () {
    ...
    function renderMovies(response) {//mostrar datos
        ...
        for (var n in response.Search) {
            ...
            if (movie.Poster !== "N/A") {
                movieList.append(movieSearch(movie));
            }
            else {
                movieList.append(movieSearch(movie));
            }
    ...
    function cardMoviesNew(response) {
        for (var m in response.Search) { //recorrer el objeto retornado
            ...
            if (movie.Poster !== "N/A") {
                cards.append(movieSearchCardsNews(movie));
            }
            else {
                cards.append(movieSearchCardsNews(movie));
            }
    ...
    function cardMoviesSeries(response) {     // validamos la api retorna el url sino no para agregarle una de que la imagen no se encientra disponible.      
        for (var m in response.Search) { //recorrer el objeto retornado
            ...
            if (movie.Poster !== "N/A") {
                cards2.append(movieSearchCardsSeries(movie));
            }
            else {
                cards2.append(movieSearchCardsSeries(movie));
            }
    ...
    function movieSearchCardsNews(movie) {
        let element = `
            <a class="placelist" data-name="${movie.Title}">
            <img src="${movie.Poster}" width="50px" onerror="this.onerror=null;this.src='https://lecicmaderas.com/wp-content/uploads/2017/05/imagen-no-disponible.jpg';">
            </a>
            `;
        return element
    ...
    function movieSearchCardsSeries(movie) {
        let element = `
            <a class="placelist2" data-name="${movie.Title}">
            <img src="${movie.Poster}" width="50px" onerror="this.onerror=null;this.src='https://lecicmaderas.com/wp-content/uploads/2017/05/imagen-no-disponible.jpg';">
            </a>
            `;
        return element
    ...
    $(document).ready(function () {
        ...
        $('#imgList').on('click', '.placelist', function (event) {
            let tag = $(event.currentTarget);
            loadModal(tag.data().name);
            $('#modal').modal();
        });

        $('#imgList2').on('click', '.placelist2', function (event) {
            let tag = $(event.currentTarget);
            loadModal(tag.data().name);
            $('#modal').modal();
        });
